...
void Simple_EQAudioProcessor::prepareToPlay (double sampleRate, int samplesPerBlock)
{
    ...
    // Filters must be prepared before usage : To do this, pass a "ProcessSpec" object
        // Chains will pass this singular context to each link in the chain.
    juce::dsp::ProcessSpec spec; 

    spec.maximumBlockSize = samplesPerBlock; // Maximum number of samples

    spec.numChannels = 1; // the number of channels is 1 since we are dealing with 2 MonoChains.

    spec.sampleRate = sampleRate; // Provide the SampleRate

    // Passing the specs to the MonoChannel objects. 
        // Context is passed in the processblock
...
// Called by the host and given a buffer of **any number of channels**
    // For our stereo system, we need the right and left channels from the buffer
    // typically: Left = 0; Right = 1;
void Simple_EQAudioProcessor::processBlock (juce::AudioBuffer<float>& buffer, juce::MidiBuffer& midiMessages)
{
    ...
    // Extract the channels from the block
    auto leftBlock = block.getSingleChannelBlock(0);
    auto rightBlock = block.getSingleChannelBlock(1);

    // Create the context based on blocks
    juce::dsp::ProcessContextReplacing<float> leftContext(leftBlock);
    juce::dsp::ProcessContextReplacing<float> rightContext(rightBlock);

    // Pass the context to the chains
    leftChain.process(leftContext);
    rightChain.process(rightContext);

    // At this point, the coeffiecients are not set up. Do that to get audio
        // This requires a VST plugin to load and play audio files. Create this from the Juce tutorial page (Audio)
}
...
juce::AudioProcessorValueTreeState::ParameterLayout Simple_EQAudioProcessor::createParameterLayout() {
    juce::AudioProcessorValueTreeState::ParameterLayout layout; // This is an instance of the class

    // Note: The final parameter for each object we make is the starting / default value
    ...
    // Peak Band Quality Control : Controls tightness / width of the peak band : Abstart "Q" value
    ...
    // Cut  Filteer: Mainly in Decibles / Octaves = Results in multiples of 6 or 12
        // Options are provided as a string array

    // This populates an array for strings with 12 unit increments
    juce::StringArray stringArray;
    for (int i = 0; i < 4; i++) {
        juce::String str; // holds the string
        str << (12 + i * 12); // Passing the values to the string?
        str << " db/Oct";
        stringArray.add(str); // Left off at 28:08
    }
